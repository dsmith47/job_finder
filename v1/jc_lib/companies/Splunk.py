...
class SplunkCrawler(AbstractCrawler):
  COMPANY_NAME = "Splunk"
  JOB_SITE_URLS = [ # Americas Jobs
    "https://www.splunk.com/en_us/careers/search-jobs.html?start=24&team=Engineering&region=Americas"
    ]

  def __init__(self, present_time, driver=None):
    super().__init__(present_time,
     SplunkCrawler.COMPANY_NAME,
     "https://www.splunk.com/en_us/careers/jobs/", 
     SplunkCrawler.JOB_SITE_URLS,
     has_post_processing=True,
     driver=driver)
    self.next_page = self.VISIT_ONCE
    self.load_page_content = lambda u: self.REPEATED_BUTTON_PRESS("Load More Resuls", u)
    self.engine = SeleniumEngine(self.driver)

  def extract_job_elems_from_page(self, url):
    report_items = []
    time.sleep(1)
    job_elems = self.driver.find_elements(By.XPATH, "//*[contains(@href, '{}')]".format("/en_us/careers/jobs/"))
    for e in job_elems:
